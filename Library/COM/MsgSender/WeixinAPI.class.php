...
class WeixinAPI {
    ...
    public function createMenu($data)
    {
        ...
        $result = SysUtil::curlRequest(self::API_URL_PREFIX.self::MENU_CREATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
        if($result)
        {
    ...
    public function getMenu()
    {
        ...
        $result = SysUtil::curlRequest(self::API_URL_PREFIX.self::MENU_GET_URL.'access_token='.$this->access_token);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function deleteMenu()
    {
        ...
        $result = SysUtil::curlRequest(self::API_URL_PREFIX.self::MENU_DELETE_URL.'access_token='.$this->access_token);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function getUserInfo($openid)
    {
        ...
        $result = SysUtil::curlRequest(self::API_URL_PREFIX.self::USER_INFO_URL.'access_token='.$this->access_token.'&openid='.$openid);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function getUserList($next_openid='')
    {
        ...
        $result = SysUtil::curlRequest(self::API_URL_PREFIX.self::USER_GET_URL.'access_token='.$this->access_token.'&next_openid='.$next_openid);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function getGroup()
    {
        ...
        $result = SysUtil::curlRequest(self::API_URL_PREFIX.self::GROUPS_GET_URL.'access_token='.$this->access_token);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function createGroup($name)
    {
        ...
        $result = SysUtil::curlRequest(self::API_URL_PREFIX.self::GROUPS_CREATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function updateGroup($id, $name)
    {
        ...
        $result = SysUtil::curlRequest(self::API_URL_PREFIX.self::GROUPS_UPDATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function updateGroupMembers($openid, $to_groupid)
    {
        ...
        $result = SysUtil::curlRequest(self::API_URL_PREFIX.self::GROUPS_MEMBERS_UPDATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function sendCustomMessage($touser, $data, $msgType = 'text')
    {
        ...
        $result = SysUtil::curlRequest(self::API_URL_PREFIX.self::MESSAGE_CUSTOM_SEND_URL.'access_token='.$this->access_token, $this->jsonEncode($arr), 'post');
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function checkAuth()
    {
        ...
        // 从缓存中获取access_token
        $cache_flag = 'weixin_access_token_' . $this->appid;
        $access_token = $cache->get('Weixin', $cache_flag);
        if($access_token) 
        {
        ...
        $result = SysUtil::curlRequest(self::API_URL_PREFIX.self::AUTH_URL.'appid='.$this->appid.'&secret='.$this->appsecret);
        if($result)
        {
            $jsonArr = json_decode($result, true);
    ...
    {
        $result = SysUtil::curlRequest(self::SNS_OAUTH_ACCESS_TOKEN_URL.'appid='.$this->appid.'&secret='.$this->appsecret.'&code='.$code.'&grant_type=authorization_code');
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    {
        $result = SysUtil::curlRequest(self::SNS_OAUTH_REFRESH_TOKEN_URL.'appid='.$this->appid.'&grant_type=refresh_token&refresh_token='.$refresh_token);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    {
        $result = SysUtil::curlRequest(self::SNS_USERINFO_URL.'access_token='.$access_token.'&openid='.$openid);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function createQrcode($scene_id, $type=0, $expire=1800)
    {
        ...
        $result = SysUtil::curlRequest(self::API_URL_PREFIX.self::QRCODE_CREATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
