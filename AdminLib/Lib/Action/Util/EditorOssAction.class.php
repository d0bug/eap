<?php
class EditorOssAction extends Action {
	public function upload_json_oss(){
		set_time_limit(0);
		...
		//最大文件大小
		$max_size = C('MAX_UPLOAD_FILE_SIZE');
		...
		if (empty($_FILES) === false) {
			...
			$bucket = C('BUCKET');
			$object = C('OSS_IMG_PATH').date('Y-m-d').'/'.$file_name;
			...
			if($target_file){
				...
				if($upload_file_by_content->status == 200){
					$file_url = "http://".C('DEFAULT_OSS_HOST')."/".C('BUCKET')."/".$object;
				}else{
					$this->alert("上传失败");
				}
			}else{
	...
	public function file_manager_json_oss(){
