...
class VipOperationModel extends Model {
	public  $dao = null;
	public function __construct(){
        ...
        $this->vip_operation_video = 'vip_operation_video';
        $this->vip_opvideo_type = 'vip_opvideo_type';
    ...
    public function add_video($arr,$user_key){
		if(!empty($arr['video_url']) && !empty($arr['title']) && !empty($arr['introduce'])){
			$sql = 'INSERT INTO '.$this->vip_operation_video.' (video_url,one_video_url,video_type,
													 title,
													 introduce,
													 create_time ,
													 create_name) 
    ...
    public function get_Opvideo_Type(){
            return $this->dao2->getAll("select * from ".$this->vip_opvideo_type." where status = 1" );

    }
    
    public function get_OperationList($condition='',$currentPage=1,$pageSize=20){
        $count = $this->get_OperationCount($condition);
		$pageCount = ceil($count / $pageSize);
		if($currentPage > $pageCount) $currentPage = $pageCount;
		if($currentPage < 1) $currentPage = 1;
		$strQuery = 'SELECT  *  FROM ' . $this->vip_operation_video . ' WHERE 1=1 ';
		if($condition) {
			$strQuery .= $condition." ";
		}
     ...
     public function get_OperationCount($condition='') {
		$strQuery = 'SELECT id FROM ' . $this->vip_operation_video . ' WHERE 1=1 ';
		if ($condition) {
			$strQuery .=  $condition;
		}
    ...
    public function get_videoInfo($arr){        
        $sqlQuery = 'SELECT * FROM ' . $this->vip_operation_video . ' WHERE 1=1 ';
		if(!empty($arr['id'])){
			$sqlQuery .= ' AND id = '.$arr['id'];
		}
		$videoInfo = $this->dao2->getRow($sqlQuery);        
        if(!empty($videoInfo['video_type'])){
            $typeInfo = $this->getVideoTypeRow($videoInfo['video_type']);
            $videoInfo['video_type_name'] = $typeInfo['type_name'];
        }        
        return $videoInfo;
        
    }

    public function getVideoTypeRow($video_type){
        return $this->dao2->getRow("select * from ".$this->vip_opvideo_type." where id = ".$video_type." ");
    }


    public function update_video($arr){        
        if(!empty($arr['title']) && !empty($arr['introduce'])){
            $video_url = '';
            $one_video_url = '';
            if($arr['video_url']){
			...
			$sql = 'UPDATE '.$this->vip_operation_video.' SET   video_url = '.$video_url.',
    ...
    public function deleteOperationByID($id){
        return $this->dao2->execute('update '.$this->vip_operation_video.' set status = 0 where id ='.$id);
    }
        
	public function add_video2($arr,$user_key){
	...
	public function update_video2($arr,$vid){
		if(!empty($arr['video_url']) && !empty($arr['title']) && !empty($arr['attribute_one']) && !empty($arr['attribute_two']) && !empty($arr['duration']) && !empty($arr['introduce'])){
	...
	public function get_videoInfo2($arr){
