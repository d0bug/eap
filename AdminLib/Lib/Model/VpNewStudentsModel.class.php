...
class VpNewStudentsModel extends Model {
	...
	public function __construct(){
		...
		$this->vip_program_lesson="vip_program_lesson";
		$this->vip_program_plan="vip_program_plan";
	...
	/**
	 * 课程规划列表
	 * @param  [type] $student_code [description]
	 * @param  [type] $teacher_code [description]
	 * @return [type]               [description]
	 */
	public function getProgramList($arr,$currentPage=1, $pageSize=20)
	{
		$count = $this->getProgramCount($arr);
		$pageCount = ceil($count / $pageSize);
		if($currentPage > $pageCount) $currentPage = $pageCount;
		if($currentPage < 1) $currentPage = 1;
		if(!empty($arr['student_code'])){
			$strQuery = 'SELECT id,
								...
								addtime,
								updatetime
								FROM '.$this->vip_program_plan.' 
							    WHERE student_code = '.$this->dao->quote($arr['student_code']);
			if(!empty($arr['teacherCode'])){
				$strQuery .= ' AND teacher_code = '.$this->dao->quote($arr['teacherCode']);
			}
			$list = $this->dao2->getLimit($strQuery, $currentPage, $pageSize, $order);
			return $list;
		}
		return false;
	}
	 ...
	 * @return [type]      [description]
	 */
	public function getProgramCount($arr)
	{
		$strQuery="select count(1) from ".$this->vip_program_plan." where teacher_code=".$this->dao->quote($arr['teacherCode'])." and student_code=".$this->dao->quote($arr['student_code']);
	...
	public function getprogramInfoById($program_id){
		if(!empty($program_id)){
			$row = $this->dao2->getRow('SELECT * FROM '.$this->vip_program_plan.' WHERE id = '.$this->dao->quote($program_id));
			if(!empty($row)){
				$row['program_lesson'] = $this->dao2->getAll('SELECT lesson_no,lesson_topic,lesson_major FROM '.$this->vip_program_lesson.' WHERE program_id = '.$this->dao2->quote($program_id));
			}
			return $row;
		}
		return false;
	 ...
	 * @param [type] $arr [description]
	 */
	public function addVipProgram($arr){
		$now = date('Y-m-d H:i:s');
		$strQuery = 'INSERT INTO '.$this->vip_program_plan.' ( student_code,
															      student_name,
															      addtime,
															      kecheng_code,
															      kecheng_name,
															      ...
															      dept_code,
															      dept_name)
														VALUES ('.$this->dao2->quote($arr['student_code']).',
																'.$this->dao2->quote($arr['student_name']).',
																'.$this->dao2->quote($now).',
																'.$this->dao2->quote($arr['kecheng_code']).',
																'.$this->dao2->quote($arr['kecheng_name']).',
																'.$this->dao2->quote($arr['grade_name']).',
																'.$this->dao->quote($arr['new_program_img']).',
																'.$this->dao->quote($arr['new_program_file']).',
																'.$this->dao2->quote($arr['classadviser_name']).',
																'.$this->dao2->quote($arr['teacher_code']).',
																'.$this->dao2->quote($arr['teacher_name']).',
																'.$this->dao2->quote($arr['dept_code']).',
																'.$this->dao2->quote($arr['dept_name']).'
																)';
		$this->dao2->execute ( 'begin' ); // 事务开启
		$success = (boolean)$this->dao2->execute($strQuery);
		$new_program_id = $this->dao2->getOne('SELECT id FROM '.$this->vip_program_plan.' ORDER BY id DESC ');
		$strQuery2 = '';
		foreach ($arr['programLesson'] as $key=>$lesson){
			$strQuery2 .= 'INSERT INTO '.$this->vip_program_lesson.' (program_id,
																	 lesson_no,
																	 lesson_topic,
																	 lesson_major,
																	 addtime) 
															VALUES ('.$this->dao2->quote($new_program_id).',
																	'.$this->dao2->quote($lesson['lesson_no']).',
																	'.$this->dao2->quote($lesson['lesson_topic']).',
																	'.$this->dao2->quote($lesson['lesson_major']).',
																	'.$this->dao2->quote($now).');';
		}
	 ...
	 * @return [type]             [description]
	 */
	public function editVipProgram($program_id,$arr)
	{
		if(!empty($program_id) && !empty($arr)){
				$now = date('Y-m-d H:i:s');
				$strQuery = 'UPDATE '.$this->vip_program_plan.' SET program_html = '.$this->dao->quote($arr['new_program_file']).',
																   program_img = '.$this->dao->quote($arr['new_program_img']).',
																   updatetime = '.$this->dao->quote($now).' WHERE id = '.$this->dao->quote($program_id);
				$strQuery2 = 'DELETE FROM '.$this->vip_program_lesson.' WHERE program_id = '.$this->dao->quote($program_id);
				$strQuery3 = '';
				foreach ($arr['programLesson'] as $key=>$lesson){
					$strQuery3 .= 'INSERT INTO '.$this->vip_program_lesson.' (
																			program_id,
																		 	lesson_no,
																		 	lesson_topic,
																		 	lesson_major,
				...
				if($success == true){
					//$this->dao->commit();
					(boolean)$this->dao2->execute($strQuery2);
					(boolean)$this->dao2->execute($strQuery3);
					return true;
				}
				//$this->dao->rollback();
				return false;
