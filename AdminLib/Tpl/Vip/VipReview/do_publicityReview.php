			...
			function(data){
				if(data == 1){
					...
					$.ajax({
						...
						dataType: 'jsonp',
						jsonp: 'callback',
						data: 'appUrl=<?php echo APP_URL?>', //请求数据
						success: function (data2) {
							if(data2.error == false){
								alert('网络宣传信息审核成功');
								window.open('http://www.gaosivip.com'+data2.url);
							}else{
								alert("网络宣传信息审核失败，"+data2.message);
							}
						},
...
<div id="main">
	<div id="search">
		<form id="getPublicityform" name="getPublicityform" method="POST" action="<?php echo U('Vip/VipReview/publicityReview')?>">
	...
	<h2>网络宣传信息审核</h2>
	<div id="return"></div>
	<form id="publicity_form" name="publicity_form" method="POST"  enctype="multipart/form-data" >
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableInfo">
		...
		<tr><td class="alt"><font color=red>*</font>头　像：</td>
			<td class="no-border-right">
				...
				<!-- <a href="javascript:$('#file_upload_avatar').uploadify('upload','*');">上传</a> -->
				<div id="view_img_one">
			...
			<td width="60%" class="no-border-left">注：<br>
					...
					3. 图片大小不要大于3兆 单位(M / 1M=1024KB 1KB=1024B)<br>
			</td>
		</tr>
		...
		<tr><td></td>
			<td colspan="2">
				...
				<a href="#" onclick="preview('<?php echo U('Vip/VipInfo/preview')?>')">预览</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="<?php echo U('Vip/VipInfo/not_passReview',array('pid'=>$publicityInfo['pid'],'user_key'=>$publicityInfo['user_key']))?>" class="error">不通过</a>
			</td>
		</tr>
		<?php endif;?>
	</table>
