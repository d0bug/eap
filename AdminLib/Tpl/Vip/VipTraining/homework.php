...
<body style="padding: 5px;">
	<div id="basic-dict-layout" class="easyui-layout" data-options="border: false, fit: true">
		<div region="west" style="width: 300px;" data-options="title:'作业管理', iconCls:'icon-table', split: true, minWidth: 800">
			<table id="easyui-datagrid" class="easyui-datagrid" data-options="url: '/Vip/VipTraining/homeworkInfo',
																	  iconCls: 'icon-table',
																	  striped: true,
				...
				<thead>
					<tr>
						...
						<th field="hw_name" width="50px">作业名称</th>
                        <th field="zujuan" width="50px">组卷</th>
                        <th field="tr_name" width="250px">培训名称</th>
                        <th field="zongrenshu" width="50px">总人数</th>
                        <th field="wan" width="50px">完成人数</th>
                        <th field="wei" width="50px">未完成完成人数</th>
                        <th field="recommended_name" width="50px">是否推送</th>
                        <th field="chakan" width="50px">详情</th>
					</tr>
				</thead>
			</table>
		</div>
...
<!--End 基础属性添加/编辑对话框-->
	...
	<script language='javascript' type='text/javascript'>
        ...
        function add_dict(type){
            var params = {
    			href: '/Vip/VipTraining/addHomework',
    			iconCls: 'icon-add',
    			title: '添加'
    		};
    		if (type == 'PEIXUN') {
       ...
       function edit_dict() {        
		...
		params.href = '/Vip/VipTraining/addHomework?id=' + row.id;
		$('#basic-index-knowledge-add-dlg').dialog(params).dialog('open');
		//action = '/Vip/VipTraining/dict_add_people';
		form = 'dict-edit-form';
      ...
      function easyui_delete() {
			...
			if (row) {
				$.messager.confirm('操作提示', '您确实要删除吗？', function (r) {
					if (r) {
						$.post('/Vip/VipTraining/delHomework', { id: row.id }, function (result) {
							if (result.status) {
        ...
        function xiangqing(id){
            var params = {
                height: 500,                
                href:'/Vip/VipTraining/homeworkDetailedList?id='+id,
                iconCls: 'icon-add',
                title:'详情',
            };
            $('#basic-index-knowledge-add-dlg').dialog(params).dialog('open');
