...
<div class="easyui-layout" data-options="fit: true">
	<div region="north" data-options="fit: true, collapsible: false, border: false">
		<form id="dict-add-form" method="post" novalidate action="/Vip/VipTraining/dict_add_homework" onSubmit="return myCheck()">
        <input type="hidden" name="id" value="<?php echo $homeworkInfo['id'];?>" />
        <input type="hidden" name="trid" value="<?php echo $homeworkInfo['tr_id'];?>" />
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableInfo">
              <span style="color: red;">注:请认真填写培训名称、组卷、学科，如有误，请删除记录重新添加！</span>                 
				 <tr>
					<td class="alt right"><span class="red">*</span> 作业名称：</td>
					<td>                   
					<input type="text"  name="hw_name" class="easyui-validatebox"  value="<?php echo $homeworkInfo['hw_name'];?>" />
                ...
                <tr>
                    ...
                    <td>
                    <?php if($homeworkInfo['id'] != ''){?>
                            <?php echo $homeworkInfo['tr_name'];?>
                    <?php }else{?>
                        <select name="tr_name">
                        <?php 
                	       if(!empty($trNameInfo)){
                ...
                <tr>
                    ...
                    <?php if($homeworkInfo['zujuan'] != ''){?>
                        <td><span style="color: blue;"><?php echo $homeworkInfo['zujuan_name']?></span></td>
                    <?php }else{?>
                    <td>
                ...
                <tr>
					...
					<td>
                      <?php if($homeworkInfo['id'] == ''){?>
                      <input type="checkbox" id="selAll" name="selAll" onclick="selectAll();" />全选&nbsp;&nbsp;<span style="color: red;">如果选择全部学科，请一定要勾选全选按钮！</span>
            			...
            			if(!empty($dictInfo)){
            					foreach ($dictInfo as $k=>$module){
            						...
            						?>
                                    <?php if($homeworkInfo['id'] != ''){?>
                                    <span id="span_module_<?php echo $k+1;?>">&nbsp;&nbsp;<?php echo $name;?></span>&nbsp;&nbsp;
                                    <?php }else{?>                            
                    ...
                    <?php }else{?>
                    <?php echo $homeworkInfo['xueke_name'];?>
                    <?php }?>
					</td>
				</tr>
                 <?php
            		$ckd = $chkdd = '' ;            		
            		if((!empty($homeworkInfo) && $homeworkInfo['recommended'] == 1 ))
            			$ckd = 'checked';
            		if(!empty($homeworkInfo) && $homeworkInfo['recommended'] == 2)
            			$chkdd ='checked';		
        		?>
				<tr>
