...
<div class="fd-top">
	<div class="fd-wauto fd-top-bg"><a href="index.html" class="fd-fl"><img src="/static/images/fd-01.png"></a><p class="fd-fl"><?php echo $studentInfo['sstudentname']?><i> 同学的 </i><?php echo $programInfo['subject_name']?><i> 辅导方案</i><span class="fa-time"><?php echo $date;?></span></p></div>
</div>
<div class="fd-wauto fd-con">
	<p class="fd-con-title">基本信息 </p>
	<div class="fd-content">
		<p class="fd-con-info" ><?php echo $studentInfo['sstudentname']?>同学，当前为<i><?php echo $studentInfo['gradename']?> </i>，其教师为<i><?php echo $programInfo['teacher_name']?>老师</i>、学管师为<i><?php echo $studentInfo['sclassadvisername']?>老师</i>，所属校区为<i><?php echo $studentInfo['sdeptname']?></i>。</p>
	...
	<div class="fd-content clearfix ">
		<div class="fd-fl fd-con-xing fd-con-teacherxing">
			<ul>
				...
				<?php foreach ($dimensionLevel as $key=>$dimension):?>
				<li>
					<span><?php echo $dimension['title']?>：</span>
					<i id="fd-XP<?php echo $dimension['id'];?>" ></i>
					<div id="fd-XP<?php echo $dimension['id'];?>-jiantou" class="fd-hint-jiantou"></div>
					<samp id="fd-XP<?php echo $dimension['id'];?>-hint" class="fd-hint-text"></samp>
...
<bottom class="fd-button" onclick="return createProgramHtml('<?php echo U('Vip/VipStudents/createProgramHtml',array('act'=>'edit','testCoachId'=>$testCoachId,'program_id'=>$programInfo['id']))?>','<?php echo U('Vip/VipStudents/saveRaty')?>','<?php echo U('Vip/VipStudents/newStudentProgram2',array('student_code'=>$studentInfo['sstudentcode']))?>')">生成辅导方案</bottom>
</form>
...
<?php foreach ($dimensionLevel as $key=>$dimension):?>
	$('#fd-XP<?php echo $dimension['id']?>').raty({
		score:<?php echo $dimension['level']?>,
		hints:[<?php foreach ($dimension['text'] as $k=>$v):?>'<?php echo $v['text']?>',<?php endforeach;?>],
		target: '#fd-XP<?php echo $dimension['id']?>-hint',
		targetKeep: true,
		click:function(){$('#fd-XP<?php echo $dimension['id']?>-hint, #fd-XP<?php echo $dimension['id']?>-jiantou').addClass("fd-block")}
	});
<?php endforeach;?>
</script>
