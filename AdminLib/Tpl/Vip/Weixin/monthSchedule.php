...
<article class="wrap">
	<ul class="modTab">
		<li ><a href="<?php echo U('Vip/Weixin/weekSchedule',array('openid'=>$userInfo['openId']))?>">周课表</a><i></i></li>
		<li class="current"><a href="<?php echo U('Vip/Weixin/monthSchedule',array('openid'=>$userInfo['openId']))?>">月课表</a><i></i></li>
	</ul>
	<div class="courseHd">
		<label class="selbar">
		<?php if(!empty($selectData)):?>
			...
			<?php foreach($selectData as $key=>$data):?>
				<option value="<?php echo $data['key']?>" <?php if($year.'_'.$month==$data['key']):?>selected<?php endif;?>><?php echo $data['name']?></option>
			<?php endforeach?>
			</select>
		<?php endif;?>
			<div class="arr"><span></span></div>
			<div class="txt">本周：共<?php echo $totalStudents?>人,共<?php echo $totalHours;?>小时</div>
		</label>
		<div class="addCourse">
	...
	<div class="courseList">
		...
		<?php if(!empty($monthData)):?>
			<?php foreach($monthData as $key=>$data):?>	
			<li class="row">
				<h4><i></i><span></span><?php echo $data['month']?>月<?php echo $data['day']?>日 <?php echo $data['weekStr']?><br></h4>
				<?php if(!empty($data['lesson'])):?>
					...
					<div class="stu <?php if($lesson['is_end']==1):?>finish<?php endif;?>"> 
						...
						<?php if(!empty($lesson['lesson_topic'])&&!empty($lesson['comment'])):?>
								...
								<?php if($lesson['dtdatereal'] >= C('BIOCLOCK_START') && $lesson['nstatus']==2):?>
									<?php if(empty($lesson['lesson_topic'])):?>
										<?php if($lesson['overdue']==0):?>
											<a href="<?php echo U('Vip/Weixin/keChengHeLu',array('act'=>'add','helu_id'=>$lesson['id'],'student_code'=>$lesson['sstudentcode'],'student_name'=>$lesson['sstudentname'],'kecheng_code'=>$lesson['skechengcode'],'lesson_no'=>$lesson['nlessonno'],'lesson_date'=>strtotime($lesson['dtdatereal']),'lesson_begin'=>strtotime($lesson['dtlessonbeginreal']),'lesson_end'=>strtotime($lesson['dtlessonendreal'])))?>" class="b b1">核录</a>
										<?php else:?>
										...
										<?php endif;?>
									<?php else:?>
										<!--<?php if($userInfo['user_type_key']=='VTeacher'):?>
											<a href="javascript:void(0)" class="b b2">已核录</a>
										<?php else:?>-->
											<?php if($lesson['overdue']==0):?>
												<a href="<?php echo U('Vip/Weixin/keChengHeLu',array('act'=>'update','helu_id'=>$lesson['id'],'student_code'=>$lesson['sstudentcode'],'student_name'=>$lesson['sstudentname'],'kecheng_code'=>$lesson['skechengcode'],'lesson_no'=>$lesson['nlessonno'],'lesson_date'=>strtotime($lesson['dtdatereal']),'lesson_begin'=>strtotime($lesson['dtlessonbeginreal']),'lesson_end'=>strtotime($lesson['dtlessonendreal'])))?>" class="b b1">核录</a>
											<?php else:?>
											<a href="javascript:void(0)" onclick="do_overdue('<?php echo U('Vip/Weixin/doOverdue',array('helu_id'=>$lesson['id']))?>','#popWindow3','#title3','#error_msg3',<?php if(empty($lesson['is_overdue'])):?>1<?php else:?>0<?php endif;?>)" class="b b4" >逾期</a>
											<?php endif;?>
										<!--<?php endif;?>-->
...
function changeMonth(key){
	window.location.href="<?php echo U('Vip/Weixin/monthSchedule')?>/key/"+key;
}
</script>
</body>
</html>
