...
<div id="formInfoDiv" class="easyui-layout" fit="true">
	<div region="north" style="height:30px">
		<div class="datagrid-toolbar" style="background:#eee">
			<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-save" plain="true" id="doEditAct">保存</a>
		</div>
	</div>
		...
		<div class="easyui-layout" fit="true">
			<div region="north" border="false" style="height:190px">
				...
				<ul style="margin:0px;padding-left:10px">
					<li>
					<span>标题：</span>
					<input type="text" name="act_title" id="act_title" required="true" style="width:190px" value="<?php echo $actInfo['act_title']?>" /></li>
					<li>
					<span>时间：</span>
					<input type="text" id="act_start" onkeydown="return false" size="10" name="act_start" class="easyui-datebox" value="<?php echo $actInfo['act_start']?>" /> — 
					<input type="text" id="act_end" onkeydown="return false" size="10" name="act_end" class="easyui-datebox" value="<?php echo $actInfo['act_end']?>" />
					</li>
					<li>
					<span>模板：</span>
					<input type="text" name="act_tpl" id="act_tpl" value="<?php echo $actInfo['act_tpl']?>" style="width:130px;ime-mode:disabled" />(非必填项)</li>
					<li>
					<span>年级：</span>
					<dl>
					<?php 
					...
					if($i >0):
					?>
					<dd><label><input type="checkbox" class="act_grade" value="<?php echo $gradeYear?>" <?php if(in_array($gradeYear, $actInfo['act_grades'])){echo ' checked';}?> />
			...
			<div region="center" border="false" style="border-top:1px solid #ccc;overflow:hidden">
				...
				<textarea id="act_content" style="width:100%;height:235px"><?php echo $actInfo['act_content']?></textarea>
...
jQuery(function(){
	jQuery.each(<?php echo $jsonAttr?>, function(k,attr){
		jQuery('#attrList').append(attr.opt_html);
		jQuery('.attrSeq_' + attr.attr_name).val(attr.attr_seq)
		jQuery('.attrType_' + attr.attr_name).val(attr.attr_type)
		jQuery('.attrCaption_' + attr.attr_name).val(attr.attr_caption)
		if(attr['attr_opts']) {
			jQuery('.item_' + attr.attr_name).find('input,textarea,select').each(function(){
				var re = new RegExp('^' + attr.attr_name + '\\[');
				if(re.test(this.name)) {
					optName = this.name.replace(attr.attr_name + '[', '').replace(']', '');
					try{
						if(this.type == 'text' || this.type == 'TEXTAREA' || this.tagName=='TEXTAREA' || this.tagName=='SELECT') {
							this.value = attr['attr_opts'][optName]
						} else if (this.type == 'checkbox') {
							if (attr['attr_opts'][optName]) {
								this.checked = true;
							}
						}
					} catch(e) {}
	...
	jQuery('#doEditAct').click(function(){
		keEditors.act_content.sync();
		actInfo = {
					actId:'<?php echo $actInfo['act_id']?>',
		...
		jQuery.post('<?php echo $editActUrl?>', formData, function(data) {
			if(data.errorMsg) {
				alert(data.errorMsg);
			} else {
				alert('微信活动修改成功');
