...
<div class="easyui-layout" fit="true" border="false">
	<div region="north" style="height:130px">
		...
		<input type="text" size="20" name="ygroup_caption" id="ygroup_caption" class="easyui-validatebox" required="true"  />
		(中文填写，仅后台可见)
		</div>
		<div class="item"><b>&nbsp;预约开始时间</b>：
		<input type="text" name="start_time" id="start_time" size="20" class="easyui-datetimebox" required="true" />
		</div>
		<div class="item"><b>&nbsp;预约结束时间</b>：
		<input type="text" name="end_time" id="end_time" size="20" class="easyui-datetimebox" required="true" />
		</div>
		<div class="item"><b>&nbsp;区分新老生<input type="checkbox" id="stu_filter" name="stu_filter" value="1" /></b>：
		报班日期区间：<input type="text" size="10" name="study_date" id="study_date_start" class="stu_filter easyui-datebox" /> - <input type="text" size="10" name="study_date_end" id="study_date_end" class="stu_filter easyui-datebox" />
		</div>
	</div>
	<div region="center">
	...
	<div region="south" style="height:30px">
		<a href="javascript:doAddGroup()" class="easyui-linkbutton" iconCls="icon-save" style="margin-top:3px">保存设置</a>
	</div>
</div>
...
function doAddGroup() {
	...
	if('' == yGroupCaption) {
		...
		if(0 == seleExams.length) {
			...
			data = {eGroupId:'<?php echo $eGroupId?>', 
			...
			jQuery.post('<?php echo $addGroupUrl?>', data, function(data){
				if(data.errorMsg) {
					alert(data.errorMsg)
				} else {
					alert('预约组添加成功');
