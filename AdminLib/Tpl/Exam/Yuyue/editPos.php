...
<div class="easyui-layout" fit="true" border="false">
	<div region="north" style="height:28px">
		<a href="javascript:doEditPos()" class="easyui-linkbutton" iconCls="icon-save" plain="true">保存地点信息</a>
	</div>
	<div region="center" border="false">
		<div class="easyui-layout" fit="true" border="false">
			<div region="north" style="height:90px" border="false">
				<div class="posItem"><b>&nbsp;地点名称</b>：<input type="text" name="pos_caption" id="pos_caption" class="easyui-validatebox" required="true" value="<?php echo $posInfo['pos_caption']?>"/></div>
				<div class="posItem"><b>&nbsp;联系电话</b>：<input type="text" name="pos_caption" id="pos_telephone" class="easyui-validatebox" required="true" value="<?php echo $posInfo['pos_telephone']?>" /></div>
				<div class="posItem"><b>&nbsp;地址</b>：<input size="40" type="text" name="pos_caption" id="pos_addr" class="easyui-validatebox" required="true" value="<?php echo $posInfo['pos_addr']?>" /></div>
			</div>
			<div region="center">
...
var posCodes = '<?php echo $posInfo['pos_epos_list']?>'.split(',');
jQuery(function(){
	jQuery('#ePosGrid').datagrid({
		...
		onLoadSuccess:function(){
			var rows = jQuery('#ePosGrid').datagrid('getRows');
			jQuery(rows).each(function(idx, pos){
				if(inArray(pos.pos_code, posCodes)) {
					jQuery('#ePosGrid').datagrid('selectRow', idx)
				}
			})
		}
	})
...
function doEditPos() {
	...
	if(valid) {
		...
		if(selections.length ==0) {
			...
			var data = {pos_id:'<?php echo $posInfo['pos_id']?>',
						yGroupId:'<?php echo $posInfo['ygroup_id']?>',
						...
						posList:posList}
			jQuery.post('<?php echo $editPosUrl?>', data, function(data){
				if(data.errorMsg) {
					alert(data.errorMsg);
				} else {
					alert('诊断地点信息修改成功');
