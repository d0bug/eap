...
<html>
    <head>
		...
		function setBlankCfg() {
		  var blankCnt = Math.abs(jQuery('#blank_cnt').val());
		  var curCnt = jQuery('#rightTable').find('tr').length;
		  if(blankCnt >= curCnt) {
		      for(var i=curCnt;i<blankCnt;i++)  {
		          var blankIdx = (i+1);
		          jQuery('<tr><td style="width:230px" valign="top">第<span style="font-size:16px;color:red">' + blankIdx + '</span>空,正确答案项<input type="text" style="width:60px;text-align:center;font-weight:bold" name="ansCnt[' + blankIdx + ']" blankIdx="' + blankIdx + '" value="1" min="1" id="ansCnt_' + blankIdx + '" />个</td><td id="rightAnsItems_' + blankIdx + '" valign="top"><div>答案值：<input size="50" type="text" name="ques_answer_items[' + blankIdx + '][]" /></div></td></tr>').appendTo('#rightTable');
		          jQuery('#ansCnt_' + blankIdx).numberspinner({
		              onChange:function(){
		                  setRightAnsItems(this)
		              }
		          });
		          jQuery('<tr><td style="width:230px" valign="top">第<span style="font-size:16px;color:red">' + blankIdx + '</span>空，错误答案项<input type="text" style="width:60px;text-align:center;font-weight:bold" name="wrongCnt[' + blankIdx + ']" blankIdx="' + blankIdx + '" value="0" min="0" id="wrongCnt_' + blankIdx + '" />个</td><td valign="top" id="quesWrongItems_' + blankIdx + '"></td></tr>').appendTo('#wrongTable');
        ...
        jQuery(function(){
             quesBodyBtn();
             setBlankCfg();
             jQuery('#blank_cnt').numberspinner({
                 onChange:function(){
                    setBlankCfg();
                 }
             })
    ...
    <body class="easyui-layout" fit="true" border="false">
        <div region="north" style="height:165px;">
            ...
            <table class="quesInfo" width="90%" align="center" cellpadding="2" cellspacing="0">
                ...
                <tr><th>填空个数：</th><td><input type="text" value="1" id="blank_cnt" min="1" max="10" /></td><th>难度级别：</th><td><input type="text" value="1" class="easyui-numberspinner" min="1" max="5" name="ques_level" /></td></tr>
            </table>
        </div>
        <div region="center">
