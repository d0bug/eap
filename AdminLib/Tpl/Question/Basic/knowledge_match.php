...
<body>
	<div id="basic-index-question-layout" class="easyui-layout" fit="true">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableInfo">
					...
					<tr>
						...
						<td>
							...
							<a href="#" id="knowledge_select" class="blue ">选取五级知识点</a>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
                           ...
                           <!-- <textarea name="analysis2" id="analysis2" style="width:90%; height:160px" placeholder="输入内容..."></textarea>
                            <?php echo W('EditorOss', array('id'=>'analysis','layout'=>'basic'))?>-->
                            <!-- 'layout'=>'default' 值为default启用默认样式， 值为'simple' 启用简单样式-->
                            <textarea id="knowledge_analysis_{$nextKnowledgeId}" name="analysis" class="editor"></textarea> <!--编辑器替换为KindEditor编辑器 2015 03 10-->
		...
		<div region="south" style="height: 32px;" data-options="collapsible: false, border: false, split: false">
			<a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon-save'" onclick="knowledge_match_save()">保存</a>
		</div>
	</div>
	...
	<script language='javascript' type='text/javascript'>
			...
			$('#knowledge_select').tooltip({
				...
				onUpdate: function(content){
					content.panel({
						...
						border: false,
						cache: false,
						href: '/Question/Basic/knowledges3?coursetypeid={$coursetypeid}&is_gaosi=1'
					});
				},
				onShow: function(){
					var t = $(this);
		...
		function knowledge_match_save() {
			$('#knowledge-add-form').form('submit', {
				url: '/Question/Basic/knowledge_match_save',
				onSubmit: function () {
				...
				success: function (result) {
					...
					if (result.status) {
						...
						parent.$('#basic-index-knowledge-match-dlg').dialog('close');
					} else {
