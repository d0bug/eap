...
<else />
	...
	<div class="qt_box mr_b_5">
		...
		<div class="info" style="padding: 5px 0 0 10px;">
			<span style="float:left;" class="blod">ID：{$question['question_id']}</span>
			<span style="float:left; margin-left: 25px;" class="blod">题号：{$question['number']}</span>
			<if condition="($question['is_classic'] eq 1)"><span style="float:left; margin-left: 25px;" class="blod red">经典题</span></if>
			<if condition="($question['difficulty'] gt 0)"><span style="float:left; margin-left: 25px;" class="blod">难度：{$question['difficulty'] | get_difficulty_name}</span></if>
			<if condition="($question['knowledge_name'] neq '')"><span style="float:left; margin-left: 25px;" class="blod">主知识点：{$question['knowledge_name']}</span></if>
			<span style="float:left; margin-left: 25px;" class="blod">修改时间：{$question['knode_last_updated_time']}</span>
			<if condition="($question['is_content_error'] eq 1)"><span style="float:right; margin-right: 10px;" class="blod red">题干有问题</span></if>
		</div>
