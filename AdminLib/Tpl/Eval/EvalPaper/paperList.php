	...
	<div id="list" class="clearfix">
		...
		<table width="70%" border="1">
			...
			<tr height=30>
				...
				<td align="center"><a href="#" onclick="testMessageBox_showAnalysis(event,'<?php echo $question['show_analysis'];?>','<?php echo $question['analysis_width']?>','<?php echo $question['analysis_height']?>')"><img src="<?php echo $question['show_analysis'];?>" width="200" height="150"></a></td>
				<td align="center"><?php echo $question['paper_title'];?></td>
				<td align="center"><?php echo $question['module_name'];?></td>
				<td align="center">
				<?php foreach($difficulty as $key=>$d):?>
					<?php 
					if($key == $question['difficulty'])
						echo $d;
					?>
				<?php endforeach?>
				</td>
				<td align="center"><?php echo $question['answer'];?></td>
				<td align="center"><?php echo $question['score'];?></td>
				<td align="center"><?php echo $question['seq'];?></td>
				<td align="center">
				<?php
					if($question['is_key']==1)
						$chk ='checked';
					else
						$chk = '';
				?>
					
				 	<input type="checkbox" name="key" id="key" value="1" <?php echo $chk;?> onclick="testMessageBox_questionInfo(<?php echo $question['module_id'];?>,<?php echo $question['id'];?>,'<?php echo U('Eval/EvalPaper/key_question')?>',this.value=this.checked?1:0)" >&nbsp;&nbsp;设为关键题&nbsp;&nbsp; |&nbsp;&nbsp; 
					<a href="#" onclick="testMessageBox_statisticInfo(event,'<?php echo U('Eval/EvalPaper/statisticInfo',array('id'=>$question['id']))?>')" class="blue">查看统计</a>&nbsp;&nbsp; |&nbsp;&nbsp; 
					<a href="#" onclick="testMessageBox_questionForm(event,'edit','<?php echo U('Eval/EvalPaper/addQuestion',array('id'=>$question['id']))?>')" class="blue">修改</a>&nbsp;&nbsp; | &nbsp;&nbsp;
					<a href="<?php echo U('Eval/EvalPaper/deleteQuestion',array('id'=>$question['id']))?>" class="blue" onclick="return confirm('删除该试题将同时删除该试题的所有答题记录，\n确认要删除该试题吗？')">删除</a>
				</td>
			</tr>
			<?php endforeach?>
		</table>
