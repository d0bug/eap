...
function Header(calendar, options) {
	...
	function renderSection(position) {
		...
		if(position=='right'){
			e.append("<span id='total'><label id='w_or_m'></label>：<label id='total_detail'>共0人，0小时</label></span>");
		}
		if(position=='center'){
			e.append("<span id='print'><input type=button value=\"　打印　\" onclick=\"javascript:window.print();\"></span>");
		}
...
function BasicView(element, calendar, viewName) {
	...
	function buildCellHTML(date) {
		...
		/*if(date.getFullYear()>=$('#year').val()&&(date.getMonth()+1>$('#month').val()||(date.getMonth()+1==$('#month').val()&&date.getDate()>=$('#day').val()))){
			html +="<span class='add-event'><a href='#none'>+</a></span>";
		}*/
		html +=
			"</div>" +
			"</div>" +
